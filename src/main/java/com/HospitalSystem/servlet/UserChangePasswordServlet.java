...
public class UserChangePasswordServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            RequestDispatcher dispatcher=request.getRequestDispatcher("ChangeUserPassword.jsp");
           ...
           if(!changed){
                out.println(
                        "<div class=\"alert alert-danger mb-0\"> Could not change password.Please try again!</div>"
                );
                dispatcher.include(request, response);
            }else{
                out.println(
                        "<div class=\"alert alert-success mb-0\"> Password Change.</div>"
                );
                dispatcher.include(request, response);
            }
